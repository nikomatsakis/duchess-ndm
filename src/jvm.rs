use crate::{
    ...
    thread,
    try_catch::TryCatch,
    AsJRef, ClassDefinition, Error, Global, GlobalResult, IntoRust, Local, ToJava, TryJDeref,
};
...
pub trait JvmRefOp<T: JavaObject>: Clone {
    ...
    fn into_as_jref<'jvm>(self, jvm: &mut Jvm<'jvm>) -> crate::Result<'jvm, Self::Output<'jvm>>;
}
    ...
    for<'jvm> <J as JvmOp>::Output<'jvm>: AsJRef<T>,
{
    ...
    fn into_as_jref<'jvm>(
        self,
        jvm: &mut Jvm<'jvm>,
    ) -> crate::Result<'jvm, <Self as JvmRefOp<T>>::Output<'jvm>> {
...
impl<'jvm> Jvm<'jvm> {
    ...
    pub fn define_class(&mut self, class: &ClassDefinition) -> crate::Result<'jvm, ()> {
        unsafe {
            self.0
